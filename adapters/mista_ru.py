...
from page import Page, Item, RSSPage  # pylint: disable=C0413
import utils
...
class Forum_(RSSPage):
    ...
    def from_raw(cls, raw: Any) -> Item:
        selector = Selector(text=raw, type="html")
        for topic in selector.xpath(query="//tr[starts-with(@class,'forum-')]"):
            ...
            page = cls.from_url("https://forum.mista.ru/" + href)
            guid = link
    ...
    def parse_page(self) -> tuple[str, Iterable[dict[str, Any]]]:
        """
        return (title:str, Iterable[dict[str,Any]]
        """
        pass
